...
**Per-coin close tiers** (`--coin-close-tier-overrides`): Override the breakeven/aggressive tier transition fractions per coin, e.g. `"NVDA:0.30/0.55,xyz:XYZ100:0.40/0.65"` (format `COIN:BREAKEVEN/AGGRESSIVE`, requires `0 <= breakeven < aggressive <= 1`). Lets coins with heavy adverse selection concede to breakeven earlier — improving the odds of a maker exit before the taker force-close — while calmer coins keep the wider take-profit window (values above the global defaults relax instead of tighten). Unspecified coins use `--close-breakeven-pct` / `--close-aggressive-pct`. **Toxicity-linked tier acceleration** (`--close-tier-toxicity`): dynamically multiplies a coin's tier fractions by `--close-tier-toxicity-multiplier` (default 0.6) while its recent-window average markout from the AdverseSelectionTracker is at or below `--close-tier-toxicity-threshold-bps` (default -2.0); requires `--enable-adverse-selection-log` + WebSocket and falls back to base behaviour whenever the tracker or its samples are unavailable. Bounded by `--close-tier-toxicity-floor-pct` (fraction floor) and `--close-tier-min-seconds` (absolute floor on the breakeven transition, recommended when combining with dynamic position age). Neither feature changes the force-close or taker-fallback timing itself.
...
strategies:
  ...
  market_making:
    ...
    close_tier_min_seconds: 0          # --close-tier-min-seconds  (absolute floor in seconds for breakeven transition; 0 = disabled)
    coin_close_tier_overrides: ""      # --coin-close-tier-overrides  (per-coin tier fractions: "NVDA:0.30/0.55,xyz:XYZ100:0.40/0.65")
    close_tier_toxicity_enabled: false # --close-tier-toxicity  (accelerate tiers on adverse markout; requires adverse selection log + WS)
    close_tier_toxicity_threshold_bps: -2.0  # --close-tier-toxicity-threshold-bps  (fire at/below this window-average markout; <= 0)
    close_tier_toxicity_window: "30s"  # --close-tier-toxicity-window  (sample window: 5s / 30s / 60s)
    close_tier_toxicity_multiplier: 0.6  # --close-tier-toxicity-multiplier  (tier fraction multiplier while firing; 0.1-1.0)
    close_tier_toxicity_min_fills: 5   # --close-tier-toxicity-min-fills  (min fills in window required to fire)
    close_tier_toxicity_floor_pct: 0.15  # --close-tier-toxicity-floor-pct  (breakeven fraction floor after multiplier)
...
ws_guards:                         # All require --enable-ws
  ...
  fill_feature_log_dir: "tracking/fill_features"  # --fill-feature-log-dir  (output directory; pass an absolute path in production)
  fill_feature_flush_interval: 60      # --fill-feature-flush-interval  (main-loop flush cadence in seconds)
  fill_feature_max_daily_bytes: 50000000  # --fill-feature-max-daily-bytes  (daily file size cap; excess records dropped)
  fill_feature_buffer_max: 5000        # --fill-feature-buffer-max  (in-memory record buffer cap)
